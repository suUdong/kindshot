...
from kindshot.decision import LlmTimeoutError, LlmParseError
from kindshot.models import Bucket, SkipStage


async def _run_pipeline_once(tmp_path, raw_items, decision_side_effect=None, guardrail_passed=True, dry_run=False):
    ...
    cfg = Config(log_dir=tmp_path / "logs", dry_run=dry_run)
    ...
    with patch("kindshot.main.build_context_card", new_callable=AsyncMock) as mock_ctx, \
        ...
        try:
            await asyncio.wait_for(
                _pipeline_loop(mock_feed, registry, mock_engine, market, scheduler, log, cfg, "test_run", None),
                timeout=1.0,
            )
