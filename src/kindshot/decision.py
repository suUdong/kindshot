...
_HIGH_CONVICTION_KEYWORDS: list[tuple[str, int]] = [
    ...
    ("소각 결의", 82), ("소각결의", 82), ("전량 소각", 83), ("전량소각", 83),
    ("자사주 매입", 80), ("자사주매입", 80), ("자사주 추가 매입", 80), ("자사주추가매입", 80),
    ("자기주식 취득", 80), ("자기주식취득", 80), ("자사주 추가 취득", 80),
    ("자기주식취득 신탁계약", 80),
    ("공개매수", 82), ("대항 공개매수", 84),
    ("경영권 분쟁", 82), ("경영권분쟁", 82), ("위임장 대결", 82),
    # 실적 서프라이즈 (프롬프트 83-85 → rule -2)
    ("어닝 서프라이즈", 82), ("어닝서프라이즈", 82),
    ("사상최대 실적", 83), ("사상 최대 실적", 83), ("사상최대 영업이익", 83), ("사상 최대 영업이익", 83),
    ("사상최대 매출", 83), ("사상 최대 매출", 83),
    ("역대 최대 실적", 83), ("역대 최대 영업이익", 83), ("역대 최대 매출", 83),
    ("흑자전환", 83), ("흑자 전환", 83),
    ("깜짝 실적", 82),
    # 계약/수주 — 확정 체결 패턴 (프롬프트 76-88 → rule -2)
    ("대형 계약", 81), ("대형계약", 81), ("대규모 수주", 81), ("대규모수주", 81),
    ("설계 계약", 80), ("설계계약", 80),
    ("단일판매ㆍ공급계약", 79), ("단일판매·공급계약", 79), ("단일판매", 79),
    ("규모의 공급계약", 79), ("규모 공급계약", 79),
    ("해외 수주", 80), ("해외수주", 80),
    ("방산 수주", 80), ("방산수주", 80),
    ("독점 공급", 80), ("독점공급", 80),
    ("장기 공급", 79), ("장기공급", 79),
    # 바이오 (프롬프트 82-92 → rule -4)
    ("임상 3상 성공", 88), ("임상3상 성공", 88),
    ("FDA 승인", 88), ("FDA승인", 88), ("FDA 허가", 88), ("FDA허가", 88),
    ("품목허가 승인", 82), ("품목허가 획득", 82), ("허가 획득", 82), ("허가 취득", 82), ("식약처 허가", 82),
    ("CDMO 계약", 81), ("CDMO계약", 81),
    ("기술수출 계약", 83), ("기술수출 계약 체결", 83), ("기술이전 계약", 83),
    ("라이선스 아웃", 83),
    ("임상 2상 완료", 80), ("임상2상 완료", 80), ("임상 2상 성공", 81), ("임상2상 성공", 81),
    # 특허 (프롬프트 80 → rule -1)
    ("특허 등록", 79), ("특허등록", 79), ("특허 취득", 79), ("특허취득", 79),
    ("특허 확보", 78), ("특허확보", 78),
    # M&A (프롬프트 84 → rule -2)
    ("지분 취득", 82), ("지분취득", 82),
    ("인수 완료", 82), ("인수완료", 82), ("인수 결정", 82),
    # 첫 수주/매출 — 모멘텀 전환 신호
